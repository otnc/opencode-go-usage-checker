...
export const JA: LocaleBundle = {
  ...
  wsCookieExpired:
    ...
    "新しいセッションクッキーで接続をやり直してください。",
  wsNoPayload:
    "コンソールは応答しましたが、使用量の値が含まれていませんでした。" +
    "この workspace に有効な Go プランがないか、コンソールの API が変更された可能性があります。",

  setupHeading: "まだ接続されていません",
  setupHint:
    "使用量は OpenCode のコンソールから読み取ります。そのため workspace ID と、" +
    "対応するブラウザのセッションクッキーが必要です。順番に入力を求めます。",
  setupStep1: "1. ブラウザでサインイン済みの workspace を開く",
  setupStep2: "2. アドレスバーから wrk_… の ID をコピー",
  setupStep3: "3. DevTools で `__Host-console_session` クッキーをコピー",
  ...
  wsWorkspaceIdPrompt:
    ...
    "アドレスバーの wrk_… の部分をコピーしてください: opencode.ai/console/WRK_ID/go",
  wsWorkspaceIdPlaceholder: "wrk_01ABCDEFGHIJKLMNOPQRSTUVWX",
  wsWorkspaceIdInvalid: "workspace ID の形式ではありません（wrk_… を想定しています）。",
  wsCookiePrompt: "opencode.ai の `__Host-console_session` クッキーの値",
  wsCookiePlaceholder: "クッキーの値を貼り付け",
  wsCookieRequired: "セッションクッキーの値を貼り付けてください。",
  wsHowTo:
    "サインイン済みの opencode.ai を開いた状態で F12 → Application → Storage → Cookies → " +
    "https://opencode.ai を選び、`__Host-console_session` という行の Value をコピーしてください。" +
